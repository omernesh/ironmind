...
user_setup:
  ...
  - service: hetzner
    why: "Cloud deployment automation (actual deployment in Phase 6, env setup only here)"
    env_vars:
...
must_haves:
  truths:
    ...
    - "Docker Compose services start successfully and remain healthy"
  artifacts:
...
# Hetzner - for cloud deployment (Phase 6)
# Note: INFRA-10 requirement. Actual Hetzner deployment automation is handled
# in Phase 6. For Phase 1, we only set up the environment variable placeholder.
# No deployment scripts are implemented here.
...
NOTE on INFRA-10 (Hetzner API key): This phase only sets up the environment variable
placeholder for HETZNER_API_KEY. Actual Hetzner deployment automation (provisioning
servers, deploying containers) is implemented in Phase 6 (Deployment). The .env.example
documents the variable so users can configure it when they're ready for cloud deployment.
  </action>
  <verify>
    - `cat .env.example` shows all configuration sections including HETZNER_API_KEY
    - All three .env files exist and are consistent
    - AUTH_SECRET / BETTER_AUTH_SECRET / JWT_SECRET_KEY documented to match
    - HETZNER_API_KEY comment clarifies Phase 6 deployment scope
  </verify>
  <done>Root .env.example created with all environment variables including HETZNER_API_KEY placeholder. Backend and frontend .env files updated to reference shared AUTH_SECRET. Documentation notes that secrets must match between services and that Hetzner deployment is Phase 6 scope.</done>
</task>
