...
```
  ...
  <verify>
    ...
    - `cat backend/Dockerfile` shows curl installation before COPY commands
    - `docker build -t ironmind-backend ./backend` succeeds
  </verify>
  <done>Health endpoint at GET /health returns JSON status. Dockerfile builds with Python 3.11-slim, curl installed for healthcheck, non-root user, and healthcheck configured.</done>
</task>
