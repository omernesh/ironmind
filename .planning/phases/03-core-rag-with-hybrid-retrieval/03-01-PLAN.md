...
must_haves:
  truths:
    - "Configuration includes OpenAI API key and embedding model settings"
    - "Configuration includes DeepInfra API key and reranker model settings"
    - "Configuration includes LLM model settings for GPT-5-mini"
    - "Chat request/response schemas validate required fields"
    - "Citation schema captures doc_id, filename, page_range, snippet"
  artifacts:
