...
must_haves:
  truths:
    - "POST /api/documents/upload accepts multipart file upload (backend API for INGEST-01; frontend UI is Phase 6)"
  ...
  scope_note: |
    INGEST-01 requires "User can upload a PDF via the IRONMIND interface."
    Phase 2 delivers the BACKEND API: POST /api/documents/upload endpoint.
    The FRONTEND upload UI is Phase 6 scope. This plan completes the backend contract for INGEST-01.
---

<objective>
Create document upload endpoint and docling-serve integration with async processing.

Purpose: Enable backend API to accept document uploads, validate them, store safely, and trigger async parsing via docling-serve.
...
**Scope clarification (INGEST-01):** This plan delivers the backend upload API endpoint. The frontend UI for uploading documents is delivered in Phase 6 (Frontend Integration). This plan completes the backend contract for INGEST-01.
</objective>
...
4. **Background processing function (placeholder for Plan 02-04):**
   ```python
...
- Add: `app.include_router(documents.router)`
  ...
  <done>
  - POST /api/documents/upload validates and stores files (backend API for INGEST-01)
  - Background task parses via docling-serve (placeholder, replaced by pipeline in 02-04)
...
<success_criteria>
- Upload endpoint accepts PDF/DOCX files up to 10MB (backend API for INGEST-01)
