...
<task type="auto">
  <name>Task 1: Create processing pipeline service</name>
  <files>
    backend/app/services/pipeline.py
    backend/app/routers/documents.py
  </files>
  <action>
Create complete document processing pipeline and status endpoint:
...
**backend/app/routers/documents.py (add status endpoint):**

Add GET /api/documents/{doc_id}/status endpoint:

```python
from app.services.pipeline import calculate_progress, estimate_time_remaining
...
Also update the background processing in documents.py to use DocumentPipeline:
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Complete document processing pipeline: upload, parse via docling, semantic chunk, index in txtai, status polling</what-built>
  <how-to-verify>
**Prerequisites:**
...
3. **Poll status every 2 seconds:**
   ...
   ```
   Expected: See status progress through Uploading -> Parsing -> Chunking -> Indexing -> Done
...
6. **List documents:**
   ...
   ```
   Expected: Array containing your uploaded document with status "Done"
